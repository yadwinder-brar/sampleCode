...
@Component({
  selector: 'app-sub-admin-mailbox-froward-detail',
  templateUrl: './sub-admin-mailbox-froward-detail.component.html',
  styleUrls: ['./sub-admin-mailbox-froward-detail.component.css'],
})
export class SubAdminMailboxFrowardDetailComponent implements OnInit {
  labelId = '';
  ...
  enterDetails() {
    if (this.shipmentDate) {
      ...
      this._apiService
        .addShipmentDateForForwardLabel(this.labelId, data)
        .subscribe((res) => {
          if (res?.isSuccess) {
            this._toasterService.successToast(
              'Shipment date send to user successfully'
            );
            this.getLabelDetail();
            this.isLoading = false;
          } else {
            this._toasterService.errorToast(
              'Shipment date send to user failed'
            );
            this.isLoading = false;
          }
        });
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      this.isLoading = true;
      this._apiService
        .updateForwardQuots(data, this.labelId)
        .subscribe((res) => {
          if (res?.isSuccess) {
            this.isLoading = false;
