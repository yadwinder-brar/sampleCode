...
import { UploadScanImgaesComponent } from '../upload-scan-imgaes/upload-scan-imgaes.component';
import { ToasterService } from 'src/app/core/services';
import { DomSanitizer} from '@angular/platform-browser';
import { AddMailboxCountComponent } from './add-mailbox-count/add-mailbox-count.component';
@Component({
  selector: 'app-mailboxdetail',
  templateUrl: './mailboxdetail.component.html',
  styleUrls: ['./mailboxdetail.component.css']
})
    ...
    if (this.verificationCode?.length >= 5) {
      ...
      this._apiService.verifyOtp(data,id).subscribe(res=>{
        if(res?.isSuccess){
          this.loader = false;
            this.getDetail();
            this._toasterServices.successToast('Shipment Picked Up successfully')
        }else{
          this.loader = false;
